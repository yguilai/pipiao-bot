...
import (
	...
	"github.com/yguilai/pipiao-bot/app/channelbot/internal/handler"
...
func main() {
	...
	svcCtx, err := svc.NewServiceContext(c, api)
	handleError(err)

	err = handler.RegisterNsqConsumer(svcCtx)
	handleError(err)

	intent := handler.RegisterEventHandler(svcCtx)
